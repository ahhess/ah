...
//                System.setProperty("java.util.logging.properties", "logging.properties");
    ...
    private void getEinsatz(String[] token, int iSpielernr, int iVereinsnr, int iMannschaftsnr) {
        ...
        String passnr = token[iSpielernr + 1];
        String spielername = token[iSpielernr + 3];
        String spielervorname = token[iSpielernr + 4];
        String vereinsnr = token[iVereinsnr];
        String vereinsname = token[iVereinsnr + 1];
        String mannschaftsnr = token[iVereinsnr + 3];
        String vrRrKz = token[6];
        String termin = token[8];
        ...
        if (spielernr != null && !"".equals(spielernr)
            ...
            if (spieler == null) {
                logger.warning("Unbekannte Spielernr <" + spielernr
                        + "> Name: " + spielername + ", " + spielervorname
                        + " Mannschaft: " + vereinsname + " " + mannschaftsnr);
...
//					}
//				}
                spieler = new Spieler(spielernr, passnr, spielername, spielervorname);
                spielerMap.put(spielernr, spieler);
                Verein verein = spielerMap.getVerein(vereinsnr, vereinsname, null);
                verein.getSpielerMap().put(spielernr, spieler);
                if("VR".equals(vrRrKz)) {
    ...
    public void checkErsatzspieler() throws IOException {
        ...
        for (Spieler spieler : ersatzspielerMap.values()) {
            ...
            for (int spt = 0; spt < 10; spt++) {
                ...
                if (m > 0) {
                    if (m > maxMannschaft) {
                        logger.log(Level.WARNING, "--> achtung FALSCHEINSATZ: {0}", spieler);
